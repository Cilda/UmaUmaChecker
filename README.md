...
Tesseract OCRを用いてウマ娘のウィンドウ画像からイベントを認識します。

## 目次
* [ウマウマチェッカー](#ウマウマチェッカー(UmaUmaChecker))
  * [機能](#機能)
...
ウマ娘詳細を自動選択が行われるため、変更したくない場合はストップ状態にしてください。
...
表示行数を変更することで以下のようにイベントの効果が表示されるテキストボックスの行数が調整できます。(4行から6行に変更した場合)  
